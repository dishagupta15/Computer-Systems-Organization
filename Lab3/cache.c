...
/* You need to implement these functions without changing their declarations */
void find_num_sets();
...
unsigned int num_bits_tag(){
    return (ADDRS_LENGTH - num_bits_set() - num_bits_offset() );
}
...
unsigned long find_tag(unsigned long address){
    return ( address >> ( num_bits_offset() + num_bits_set() ) );
}
